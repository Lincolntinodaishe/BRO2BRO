...
import {
  ...
  Settings, LogOut, HeartHandshake, X, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const navItems = [
  ...
  { label: "Messages",  href: "/dashboard/mentor/messages", icon: MessageCircle   },
];
    ...
    <>
      ...
      >
        <div className="h-16 border-b border-gray-100 flex items-center shrink-0 px-3 gap-2">
          <Link href="/dashboard/mentor" className={cn("flex items-center gap-2.5 flex-1 min-w-0", collapsed && "justify-center")}>
            <div className="relative shrink-0">
              <Image src={bro2broLogo} alt="BRO2BRO" height={28} className="h-7 w-auto shrink-0" />
              <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-teal-500 rounded-full border-2 border-white" />
            </div>
            <div className={cn(
              ...
              <span className="text-[10px] text-teal-600 font-semibold tracking-wide block">Mentor Portal</span>
            </div>
          </Link>
        ...
        {!collapsed && (
          <div className="mx-3 mt-3 mb-1 px-3 py-2.5 bg-teal-50 border border-teal-100 rounded-xl flex items-center gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-gray-900 truncate">Peer Mentor</p>
              <p className="text-[10px] text-teal-700">BRO2BRO Wellness Network</p>
            </div>
          </div>
        )}
        ...
        <nav className="flex-1 px-2 py-2 space-y-0.5 overflow-y-auto overflow-x-hidden">
          {navItems.map(({ label, href, icon: Icon }) => {
            const active = pathname === href || (href !== "/dashboard/mentor" && pathname.startsWith(href));
            return (
              <div key={href} className="relative group/navitem">
                ...
                >
                  <Icon className={cn("h-4 w-4 shrink-0", active ? "text-teal-400" : "text-gray-400")} />
                  <span className={cn(
          ...
          <div className="relative group/navitem">
            <Link
              ...
              className={cn(
                "flex items-center rounded-xl text-sm font-medium transition-all duration-150",
                collapsed ? "justify-center w-10 h-10 mx-auto" : "gap-3 px-3 py-2.5 w-full",
                pathname === "/dashboard/mentor/settings" ? "bg-black text-white" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"
              )}
            >
              <Settings className={cn("h-4 w-4 shrink-0", pathname === "/dashboard/mentor/settings" ? "text-teal-400" : "text-gray-400")} />
              <span className={cn(
                "overflow-hidden whitespace-nowrap transition-all duration-200",
                collapsed ? "max-w-0 opacity-0 w-0" : "max-w-xs opacity-100"
              )}>
          ...
          {collapsed ? (
            <div className="flex justify-center py-3">
              <div className="relative group/navitem">
                ...
                <Tooltip label={userName} collapsed={collapsed} />
              </div>
            </div>
          ) : (
            <div className="px-4 py-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-teal-600 mt-0.5 flex items-center gap-1">
                  ...
                  <span className="truncate">Certified Peer Mentor</span>
                </div>
              </div>
